...
def test_init():
    for provider in providers:
        provider.wipe()
    FM = FileManager(None, providers, 3, master_key, manifest_name, setup=True)
    assert len(FM.ls()) == 0
...
def test_wrong_master_key():
    for provider in providers:
        provider.wipe()
    FileManager(None, providers, 3, master_key, manifest_name, setup=True)
    with pytest.raises(exceptions.ManifestGetError):
        FM = FileManager(None, providers, 3, gen.generate_key(), manifest_name)
        FM.ls()
...
def test_wrong_manifest_name():
    ...
    FileManager(None, providers, 3, master_key, manifest_name, setup=True)
    with pytest.raises(exceptions.ManifestGetError):
        FM = FileManager(None, providers, 3, master_key, gen.generate_name())
        FM.ls()
...
def test_multiple_sessions():
    ...
    FM = FileManager(None, providers, 3, master_key, manifest_name)
    assert sorted(FM.ls()) == ["test", "test2"]
    assert FM.get("test") == "data"
